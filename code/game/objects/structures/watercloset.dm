...
/obj/structure/hygiene/sink/attackby(obj/item/hit_with, var/mob/user)
	...
	if (istype(chem_container) && ATOM_IS_OPEN_CONTAINER(chem_container) && chem_container.reagents)
		...
		chem_container.add_to_reagents(/decl/material/liquid/water/sink, min(REAGENTS_FREE_SPACE(chem_container.reagents), chem_container.amount_per_transfer_from_this))
		return TRUE
		...
		if(REAGENTS_FREE_SPACE(hit_with.reagents) >= 5)
			hit_with.add_to_reagents(/decl/material/liquid/water/sink, 5)
