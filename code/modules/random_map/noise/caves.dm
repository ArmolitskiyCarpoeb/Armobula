	...
	switch(val)
		if(1 to 4)
			if(prob(25))
				new /obj/structure/flora/plant/random_mushroom(T)
			if(prob(4))
				var/mob_type = pick(list(/mob/living/simple_animal/hostile/giant_spider))
				new mob_type(T)
		if(6 to 7)
			if(prob(8))
//				var/grass_path = pick(subtypesof(/obj/structure/flora/plant/random_mushroom/glowing))
//				new grass_path(T)
			...
			if(prob(4))
				var/mob_type = pick(list(/mob/living/simple_animal/hostile/giant_spider, /mob/living/simple_animal/hostile/slug))
				new mob_type(T)